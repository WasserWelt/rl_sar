...
RL_Real::RL_Real(int argc, char **argv)
{
    ...
    // init robot
    this->wheel_leg_sdk.InitSerial("/dev/ttyUSB0", 460800);
...
void RL_Real::GetState(RobotState<float> *state)
{
    ...
    if (ret > 0)
    {
        ...
        if (serial_dbg_cnt++ % 100 == 0) std::cout << "[Serial] Recv OK. q[0]=" << this->wheel_leg_low_state.motorState[0].q << std::endl;
        ...
        this->control.x = -((float)this->wheel_leg_joy.ly - 128.0f) / 128.0f * 1.5f;
        this->control.y = -((float)this->wheel_leg_joy.lx - 128.0f) / 128.0f * 1.5f;
        this->control.yaw = -((float)this->wheel_leg_joy.rx - 128.0f) / 128.0f * 2.0f;
